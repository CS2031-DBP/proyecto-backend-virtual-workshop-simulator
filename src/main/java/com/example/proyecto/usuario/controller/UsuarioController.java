...
public class UsuarioController {
    ...
    @GetMapping("/{usuarioId}/carreras")
    public ResponseEntity<List<Long>> obtenerCarrerasInscritas(@PathVariable Long usuarioId) {
        List<Long> carrerasInscritas = usuarioService.obtenerCarrerasInscritas(usuarioId);
        if (carrerasInscritas.isEmpty()) {
            return ResponseEntity.noContent().build();
        }
        return ResponseEntity.ok(carrerasInscritas);
    }
